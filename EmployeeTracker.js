...
const addRole = async () => {
  ...
  connection.query("SELECT * FROM departments", async (err, res) => {
    ...
    connection.query("INSERT INTO roles (title, salary, department_id) VALUES (?, ?, ?)", [title, salary, depoObj["id"]], (err, res) => {
      if (err) throw err;
      console.log(`Congrates! The new ${title} role has been added to the ${chosenDepo} department.`)
    })
    init();
  });
}
  ...
  connection.query("SELECT * FROM roles", async (err, res) => {
    ...
    if (hasManager) {
      connection.query("SELECT * FROM employees", async (err,res) => {
        if (err) throw err;
        const employeeArr = res.map(({ first_name, last_name }) => `${first_name} ${last_name}`)
        ...
        res.forEach((employee) => {
          if(chosenManager === `${employee.first_name} ${employee.last_name}`) {
            managerObj = employee;
          }
        })

  
        connection.query("INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES (?,?,?,?)", [first_name, last_name, roleObj["id"], managerObj["id"]], (err, res) => {
          if (err) throw err;
          console.log(`Congrates! You have added ${first_name} ${last_name} as an employee.`)
          init(); 
        })
      })
    } else {
      connection.query("INSERT INTO employees (first_name, last_name, role_id) VALUES (?,?,?)", [first_name, last_name, roleObj["id"]], (err, res) => {
        if (err) throw err;
        console.log(`Congrates! You have added ${first_name} ${last_name} as an employee.`)
        init(); 
      })
...
const updateEmployeeRole = () => {
  connection.query("SELECT * FROM employees", async (err, res) => {
    ...
    connection.query("SELECT * FROM roles", async (err, res) => {
      ...
      connection.query("UPDATE employees SET ? WHERE ?", [{role_id:roleObj["id"]}, {id:employeeObj["id"]}], (err, res) => {
        if (err) throw err;
        console.log(`Congrats! You have changes ${chosenEmployee}'s role to ${chosenRole}`);
        init(); 
      })
    })
  })
